...
<?php 
	...
	$vetor   = array();
		...
		if (isset($_GET["id"])){
			...
			if ($_GET["cli"] == ""){
				$sql = "SELECT c.id, c.nome, c.cpf, c.telefone, sum(d.valor) dividaTotal from cliente c left join divida d on (c.id = d.cliente_IDCliente) where ativo = 0 group by cpf ORDER BY c.nome";
				$result = $con->query($sql);
				while($row=$result->fetch_assoc()){
